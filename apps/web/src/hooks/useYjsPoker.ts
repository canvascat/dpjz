...
function readTransactions(arr: Y.Array<Y.Map<string | number>>): Array<PokerTransaction> {
  ...
  arr.forEach((yMap) => {
    txns.push({
      ...
      fromAvatarType: (yMap.get("fromAvatarType") as "text" | "notion") || "text",
      fromNotionAvatarConfig: parseNotionConfig(yMap.get("fromNotionAvatarConfig")),
...
export function useYjsPoker(roomId: string, localUser: LocalUser): UseYjsPokerReturn {
  ...
  useEffect(() => {
    ...
    const doc = new Y.Doc();
    const transactionsArray = doc.getArray<Y.Map<string | number>>("transactions");
    const membersMap = doc.getMap<Y.Map<string>>("members");
    const settingsMap = doc.getMap<number>("settings");

    // ── 同步状态的 observer ──

    const syncTransactions = () => setTransactions(readTransactions(transactionsArray));
    ...
    const registerSelf = () => {
      ...
      const notionStr = u.notionAvatarConfig ? JSON.stringify(u.notionAvatarConfig) : undefined;
    ...
    const initProviders = async () => {
      ...
      const indexeddbProvider = new IndexeddbPersistence(`dpjz-poker-${roomId}`, doc);
    ...
    if (membersMap && doc && localUser.id) {
      doc.transact(() => {
        ...
        if (localUser.notionAvatarConfig) {
          yMap.set("notionAvatarConfig", JSON.stringify(localUser.notionAvatarConfig));
        }
  ...
  const transfer = useCallback((toUserId: string, amount: number) => {
    ...
    const toAvatarType = (toMember?.get("avatarType") as "text" | "notion") || "text";
    const toNotionConfig = parseNotionConfig(toMember?.get("notionAvatarConfig"));
    ...
    doc.transact(() => {
      ...
      if (from.notionAvatarConfig) {
        yMap.set("fromNotionAvatarConfig", JSON.stringify(from.notionAvatarConfig));
      }
