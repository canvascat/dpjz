...
interface UseYjsChatReturn {
  ...
  /** 同意并发送剪切板 / 拒绝请求 */
  respondToClipboardRequest: (requestId: string, accept: boolean) => Promise<void>;
...
export async function readClipboardItem(): Promise<{
  ...
  // 优先图片，其次文本
  const preferredTypes = ["image/png", "image/jpeg", "image/gif", "image/webp", "text/plain"];
  const mimeType = preferredTypes.find((t) => item.types.includes(t)) ?? item.types[0];
  if (!mimeType) throw new Error("CLIPBOARD_EMPTY");
  const blob = await item.getType(mimeType);
  const buffer = await blob.arrayBuffer();
  return { data: new Uint8Array(buffer), mimeType };
}
export function useYjsChat(roomId: string, localUser: LocalUser): UseYjsChatReturn {
  ...
  >([]);
  const [receivedClipboard, setReceivedClipboard] = useState<ReceivedClipboard | null>(null);
  ...
  const myPendingRequestsRef = useRef<Map<string, { toNickname: string }>>(new Map());
  ...
  useEffect(() => {
    ...
    const clipboardRequests = doc.getMap<Y.Map<string | number>>("clipboardRequests");
    const clipboardResponses = doc.getMap("clipboardResponses");
    const fileSignals = doc.getMap<Y.Map<string | number>>("fileSignals");

    const parseNotionConfig = (raw: unknown): NotionAvatarConfig | undefined => {
      ...
      if (!disposed) {
        clipboardResponses.forEach((val, requestId) => {
          if (myRequestIdsRef.current.has(requestId)) {
            const toNickname = myPendingRequestsRef.current.get(requestId)?.toNickname ?? "对方";
    ...
    const cleanupStaleRequests = () => {
      ...
      clipboardRequests.forEach((val, requestId) => {
        ...
        if (typeof createdAt === "number" && now - createdAt > CLIPBOARD_REQUEST_TTL_MS) {
    ...
    const initProviders = async () => {
      ...
      const indexeddbProvider = new IndexeddbPersistence(`dpjz-chat-${roomId}`, doc);
  ...
  const requestClipboard = useCallback((toUserId: string, toNickname: string) => {
  ...
  const respondToClipboardRequest = useCallback(async (requestId: string, accept: boolean) => {
    const { doc, clipboardRequests, clipboardResponses } = yjsRef.current;
    if (!doc || !clipboardRequests || !clipboardResponses) return;
    const yMap = clipboardRequests.get(requestId) as Y.Map<string | number> | undefined;
    if (!yMap) return;
    if (!accept) {
      doc.transact(() => {
  ...
  const respondToClipboardRequestWithContent = useCallback(
    async (requestId: string, contentPromise: Promise<{ data: Uint8Array; mimeType: string }>) => {
      const { doc, clipboardRequests, clipboardResponses } = yjsRef.current;
      if (!doc || !clipboardRequests || !clipboardResponses) return;
      const yMap = clipboardRequests.get(requestId) as Y.Map<string | number> | undefined;
      if (!yMap) return;
