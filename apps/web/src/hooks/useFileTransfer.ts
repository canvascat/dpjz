  ...
  const sendFile = useCallback(
    (toUserId: string, toNickname: string, file: File) => {
      ...
      pc.createOffer()
        ...
        .catch(() => {
          setSendProgress((p) => (p?.sessionId === sessionId ? { ...p, status: "error" } : p));
      ...
      dc.onopen = () => {
        setSendProgress((p) =>
          p?.sessionId === sessionId ? { ...p, status: "sending", percent: 0 } : p,
        );
        // 发 metadata
        dc.send(
          JSON.stringify({
        ...
        const sendChunk = () => {
          if (offset >= file.size) {
            setSendProgress((p) =>
              p?.sessionId === sessionId ? { ...p, percent: 100, status: "done" } : p,
            );
            dc.close();
            pc.close();
          ...
          reader.onload = () => {
            ...
            setSendProgress((p) => (p?.sessionId === sessionId ? { ...p, percent } : p));
            sendChunk();
          };
    ...
    (sessionId: string) => {
      ...
      pc.ondatachannel = (e) => {
        ...
        dc.onmessage = (ev) => {
          ...
          if (meta) {
            ...
            const percent = Math.min(99, Math.round((received / meta.fileSize) * 100));
            setReceiveProgress((p) => (p?.sessionId === sessionId ? { ...p, percent } : p));
          }
        };
      };
      ...
      const iceObserver = () => {
        fileSignals.forEach((val) => {
          ...
          if ((sigMapInner.get("toUserId") as string) !== localUserIdRef.current) return;
      ...
      try {
        ...
        pc.setRemoteDescription(new RTCSessionDescription(desc))
          ...
          .catch(() => {
            setReceiveProgress((p) => (p?.sessionId === sessionId ? { ...p, status: "error" } : p));
            pc.close();
            sessionPeersRef.current.delete(sessionId);
          });
      } catch {
  ...
  useEffect(() => {
    ...
    const observer = () => {
      fileSignals.forEach((val) => {
        ...
        if ((rejectedMap.get("fromUserId") as string) !== localUserIdRef.current) return;
        if (!rejectedMap.get("rejected")) return;
        const sigSessionId = rejectedMap.get("sessionId") as string;
        if (!sigSessionId) return;
        setSendProgress((p) => (p?.sessionId === sigSessionId ? { ...p, status: "rejected" } : p));
