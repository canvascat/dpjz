...
function randomNickname(): string {
  ...
  const nouns = ["小猫", "小狗", "兔子", "狐狸", "熊猫", "企鹅", "独角兽", "小龙"];
 ...
 * 更新本地用户信息（合并更新）
 */
export function updateLocalUser(updates: Partial<Omit<LocalUser, "id">>): LocalUser {
  ...
  // 通知其他监听者
  window.dispatchEvent(new CustomEvent("local-user-changed", { detail: updated }));
  return updated;
}
