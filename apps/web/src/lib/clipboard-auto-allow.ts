...
export function isClipboardAutoAllow(roomId: string, fromUserId: string): boolean {
	const keys = loadKeys()
	return keys.includes(key(roomId, fromUserId))
}

export function addClipboardAutoAllow(roomId: string, fromUserId: string): void {
	const keys = loadKeys()
