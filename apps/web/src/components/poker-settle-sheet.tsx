...
import type { Balances, PokerMember, PokerTransaction } from "@/hooks/useYjsPoker";
...
function computeTeaSettleRows(transactions: Array<PokerTransaction>): Array<SettleRow> {
...
export function PokerSettleSheet({
  ...
  const plan = useMemo(() => computeSettlePlan(balances, members), [balances, members]);

  const teaRows = useMemo(() => computeTeaSettleRows(transactions), [transactions]);

  const allRows: Array<SettleRow> = useMemo(() => [...plan, ...teaRows], [plan, teaRows]);
          ...
          {hasAnySettle && (
            <ul className="space-y-3">
              {allRows.map((item, i) => (
                ...
                >
                  <span className="min-w-0 truncate text-sm font-medium">{item.fromNickname}</span>
