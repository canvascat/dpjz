...
export function ProfileSheet({
  ...
  // 草稿：仅保存后生效
  const [draftAvatarType, setDraftAvatarType] = useState<AvatarType>(user.avatarType);
  const [draftColor, setDraftColor] = useState(user.avatarColor);
  const [draftNotionConfig, setDraftNotionConfig] = useState<NotionAvatarConfig | undefined>(
  ...
  const handleSave = () => {
    ...
    if (trimmed.length > 0) {
      update({
        ...
        notionAvatarConfig: draftAvatarType === "notion" ? draftNotionConfig : undefined,
      });
    }
    setOpen(false);
  };
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent side="bottom" className="rounded-t-2xl">
        ...
        <div className="space-y-6 px-5 pt-4 pb-4">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {AVATAR_COLORS.map((color) => {
                const selected = draftAvatarType === "text" && draftColor === color;
                return (
                  <button
                    key={color}
                    type="button"
                  ...
                  >
                    {selected && <Check className="h-5 w-5 text-white drop-shadow-sm" />}
                  </button>
                );
              })}
              {/* 骰子：随机 Notion 头像 */}
          ...
          {/* 保存后头像才生效 */}
          <Button onClick={handleSave} className="w-full" size="lg" disabled={!nickname.trim()}>
            保存
          </Button>
        </div>
      </SheetContent>
