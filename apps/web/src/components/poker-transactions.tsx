...
export function PokerTransactions({
  ...
  const memberByUserId = (userId: string) => members.find((m) => m.userId === userId);
    ...
    return (
      <div className="flex h-full items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground/60">点击其他成员头像开始转分</p>
        </div>
      </div>
    );
  }
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-baseline gap-1 text-sm">
                ...
                {t.teaAmount > 0 && (
                  <span className="text-xs text-amber-600">(茶 {t.teaAmount})</span>
                )}
              </div>
