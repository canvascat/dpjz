...
export function PokerMemberBar({
  ...
  return (
    <div className="flex shrink-0 items-start gap-1 overflow-x-auto border-b px-3 py-3 sm:px-4">
      ...
      {members.map((m) => {
        ...
        return (
          ...
          >
            ...
            {/* 分数 */}
            <span className={`text-xs font-semibold tabular-nums ${scoreColor(balance)}`}>
              {formatScore(balance)}
            </span>
          </button>
        );
      ...
      {/* 分隔线 */}
      {members.length > 0 && <div className="mx-1 h-16 w-px shrink-0 self-center bg-border" />}
      ...
      >
        ...
        <span className={`text-xs font-semibold tabular-nums ${scoreColor(teaBalance)}`}>
