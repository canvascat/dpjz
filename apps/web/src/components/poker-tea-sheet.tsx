...
export function PokerTeaSheet({ open, currentRate, teaCap, onSave, onClose }: PokerTeaSheetProps) {
  ...
  const isRateValid =
    value.trim() !== "" && Number.isFinite(percent) && percent >= 0 && percent <= 100;
  const isCapValid = capValue.trim() !== "" && Number.isInteger(capNum) && capNum >= 1;
      ...
      <SheetContent side="bottom" className="rounded-t-2xl">
        ...
        <div className="space-y-4 px-5 pt-4 pb-4">
          ...
          <div className="space-y-2.5">
            ...
            <button
              ...
              className={cn(
                ...
                activeField === "cap" ? "border-ring ring-ring/50 ring-[3px]" : "hover:bg-muted/50",
              )}
            >
              {capValue || "50"}
            </button>
          ...
          <div className="space-y-2.5">
            ...
            <p className="text-xs text-muted-foreground">
              设为 0 表示不收茶位费。例如设 10%，则每转 100 分扣 10 分茶位费，接收方实得 90
              分（扣除为整数，最少 1 分）。
            </p>
          </div>
          ...
          <NumericKeypad
            ...
            showDisplay={false}
            onLimitReached={() =>
              toast.info(activeField === "rate" ? "比例最多 100%" : "累计上限最多 9999")
            }
          />
